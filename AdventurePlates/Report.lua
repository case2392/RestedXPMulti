...
function ns.BuildReport()
    ...
    line("chat links: installed=%s  filter=%s (%s)  click=%s", yn(ns.chatLinksInstalled), yn(ns.chatFilterInstalled),
         (type(ChatFrameUtil) == "table" and ChatFrameUtil.AddMessageEventFilter) and "ChatFrameUtil" or (ChatFrame_AddMessageEventFilter and "old globals" or "none"),
         table.concat({ns.linkHook or "no SetItemRef hook", ns.linkEvent or "no EventRegistry"}, "+"))
